...
| `default_server_certificate` | Certificate served by the HTTPS catch-all (default) server. The catch-all has to terminate TLS -- nginx refuses to start on an `ssl` listener with no certificate -- but it never serves content: every request is dropped with `444`. It is deliberately NOT the fqdn certificate, so no real hostname is presented for an unknown SNI. Defaults to the standalone dummy certificate written by the dehydrated role (`dehydrated.provide_default_dummy_cert`), so nothing is generated here. Set this (and `default_server_certificate_key`) to supply your own. | str | no | {{ dehydrated.prefix.dummy }}/fullchain.pem |
| `default_server_certificate_key` | Private key for `default_server_certificate`. | str | no | {{ dehydrated.prefix.dummy }}/privkey.pem |
